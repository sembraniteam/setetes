...
func (e OTPEdges) AccountOrErr() (*Account, error) {
	if e.Account != nil {
...
func (*OTP) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case otp.ForeignKeys[0]: // account_id
...
// QueryAccount queries the "account" edge of the OTP entity.
func (_m *OTP) QueryAccount() *AccountQuery {
	return NewOTPClient(_m.config).QueryAccount(_m)
}
