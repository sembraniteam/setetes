...
func (Account) Fields() []ent.Field {
	return []ent.Field{
		field.String("national_id_hash").MaxLen(64).NotEmpty().Unique().Sensitive().
		...
		field.String("country_iso_code").MaxLen(2).NotEmpty().StructTag(`json:"country_iso_code"`).
...
func (Account) Edges() []ent.Edge {
	return []ent.Edge{
		edge.To("blood_type", BloodType.Type).StorageKey(edge.Column("blood_id")).Unique().Annotations(entsql.OnDelete(entsql.NoAction)),
