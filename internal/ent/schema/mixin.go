...
func (BaseMixin) Fields() []ent.Field {
	...
	return []ent.Field{
		field.UUID("id", uuid.UUID{}).Immutable().Unique().StructTag(`json:"id"`).Annotations(entsql.Default("uuid_generate_v4()")),
		field.Int64("created_at").Positive().Immutable().StructTag(`json:"created_at"`).Default(now.UnixMilli()),
		field.Int64("updated_at").Positive().Nillable().StructTag(`json:"updated_at"`).UpdateDefault(now.UnixMilli()),
