...
func (_q *RoleQuery) QueryAccounts() *AccountQuery {
...
func (_q *RoleQuery) QueryChildren() *RoleQuery {
...
func (_q *RoleQuery) QueryParent() *RoleQuery {
	query := (&RoleClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
func (_q *RoleQuery) WithAccounts(opts ...func(*AccountQuery)) *RoleQuery {
	query := (&AccountClient{config: _q.config}).Query()
...
func (_q *RoleQuery) WithChildren(opts ...func(*RoleQuery)) *RoleQuery {
	query := (&RoleClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *RoleQuery) WithParent(opts ...func(*RoleQuery)) *RoleQuery {
	query := (&RoleClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *RoleQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Role, error) {
	...
	if query := _q.withParent; query != nil {
		if err := _q.loadParent(ctx, query, nodes,
			func(n *Role) { n.Edges.Parent = []*Role{} },
			func(n *Role, e *Role) { n.Edges.Parent = append(n.Edges.Parent, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *RoleQuery) loadAccounts(ctx context.Context, query *AccountQuery, nodes []*Role, init func(*Role), assign func(*Role, *Account)) error {
	...
	query.Where(predicate.Account(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(role.AccountsColumn), fks...))
	}))
...
func (_q *RoleQuery) loadChildren(ctx context.Context, query *RoleQuery, nodes []*Role, init func(*Role), assign func(*Role, *Role)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(role.ChildrenTable)
		s.Join(joinT).On(s.C(role.FieldID), joinT.C(role.ChildrenPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(role.ChildrenPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(role.ChildrenPrimaryKey[1]))
...
func (_q *RoleQuery) loadParent(ctx context.Context, query *RoleQuery, nodes []*Role, init func(*Role), assign func(*Role, *Role)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[uuid.UUID]*Role)
	nids := make(map[uuid.UUID]map[*Role]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(role.ParentTable)
		s.Join(joinT).On(s.C(role.FieldID), joinT.C(role.ParentPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(role.ParentPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(role.ParentPrimaryKey[0]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
